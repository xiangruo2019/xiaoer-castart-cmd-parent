...
@Component
public class ResetUserPwdHandle extends BaseHandle<ResultVo>{
	...
	public ResultVo handle(CmdVO vo) {
		...
		try {
			boolean result = commandDao.resetUserPwd(vo.getCode(), vo.getCarCard(), vo.getPassword(),vo.getNewPassword());
			if(result){
				return new ResultVo();
			}
