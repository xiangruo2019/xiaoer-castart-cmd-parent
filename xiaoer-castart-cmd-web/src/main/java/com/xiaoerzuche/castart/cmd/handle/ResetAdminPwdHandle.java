...
@Component
public class ResetAdminPwdHandle extends BaseHandle<ResultVo>{
	...
	public ResultVo handle(CmdVO vo) {
		...
		try {
			boolean result = commandDao.resetAdminPwd(vo.getCode(), vo.getCarCard(), vo.getPassword(),vo.getNewPassword());
			if(result){
				return new ResultVo();
			}
